...
function listenForNewMessages() {
  ...
  onChildAdded(messagesRef, async (snapshot) => {
    ...
    if (newMsg.type === 'received' && newMsg.msgType !== 'call' && document.visibilityState !== 'visible') {
      ...
      if (newMsg.msgType === 'photo')      notifyPhoto(otherName, otherID, photo);
      else if (newMsg.msgType === 'audio') notifyVoice(otherName, otherID, photo);
      else                                 notifyMessage(otherName, otherID, newMsg.text || '', photo);
    }
  });
}
        ...
        if (val.reactions) {
          Object.entries(val.reactions).forEach(([reactorID, emoji]) => {
            ...
            if (!notifiedReactions.has(rKey)) {
              ...
              notifyReaction(c ? c.name : otherName, reactorID, emoji, c ? c.photo : null);
            }
          });
        }
      }

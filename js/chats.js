    ...
    relevant.forEach(msg => {
      if (!localIDs.has(msg.id)) {
        ...
        if (!isFirstSync && msg.senderID !== myID && msg.msgType !== 'call') {
          ...
          if (msg.msgType === 'photo')      notifyPhoto(otherName, otherID, photo);
          else if (msg.msgType === 'audio') notifyVoice(otherName, otherID, photo);
          else                              notifyMessage(otherName, otherID, msg.text || '', photo);
        }
      }
    });
